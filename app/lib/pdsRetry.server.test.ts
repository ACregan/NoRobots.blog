...
import { NotFoundError, PdsFetchError } from "@scribe-atp/core";
...
describe("fetchWithFastPath", () => {
  ...
  it("switches to status retrying on a transient failure, and resolves once a retry succeeds", async () => {
    ...
    const assertion = result.status === "retrying" ? expect(result.data).resolves.toBe("site-data") : undefined;
  ...
  it("rejects with the last PdsFetchError once all retries are exhausted", async () => {
    ...
    const assertion = result.status === "retrying" ? expect(result.data).rejects.toThrow("PDS is down") : undefined;
    await vi.runAllTimersAsync();
    await assertion;
  ...
  it("does not retry past a NotFoundError thrown by fn on a retry attempt", async () => {
    ...
    const assertion = result.status === "retrying" ? expect(result.data).rejects.toBeInstanceOf(NotFoundError) : undefined;
    await vi.runAllTimersAsync();
    await assertion;
  });
});
