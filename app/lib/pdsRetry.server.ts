import { NotFoundError, withRetry } from "@scribe-atp/core";
...
// Attempt `fn` once, synchronously — the fast path, unchanged from today's
// behavior, so meta()/canonical/JSON-LD stay available on every normal
// request. Only on failure does this switch to streaming: the remaining
// retries run un-awaited, so the route can return a promise and let
// <Suspense>/<Await> show a spinner instead of blocking the whole response.
//
